...
class SocketIoManager
{
    ...
    public function toOrderWatchers(OrderInterface $order, $message, array $data = [])
    {
        ...
        $iri = $this->iriConverter->getIriFromItem($order);

        $channel = sprintf('orders:%s', $iri);
        $payload = json_encode([
            'name' => $messageName,
            'data' => $data
        ]);
        ...
        $this->centrifugoClient->publish(
            '$' . sprintf('%s:orders:%s', $this->namespace, $iri),
            ['event' => $payload]
        );
    }
    ...
    public function toUser(UserInterface $user, $message, array $data = [])
    {
        ...
        $this->centrifugoClient->publish(
            sprintf('%s#%s', $this->namespace, $user->getUsername()),
            ['event' => $payload]
        );
